...
source(here("r/setup_subtypes_phase_1.R"))
...
save_graph <- function(graph, name, path = "", width = 8, height = 10){
  ...
  ggplot2::ggsave(filename = paste0(name, ".png"), 
         plot = graph, 
         path = here::here("figures/first_phase", path), 
...
.corr_p1 <- plot_corr_heatmap(metrics, "pearson")
...
.corr_s1 <- plot_corr_heatmap(metrics, "spearman")
...
walk2(
  .x = list(.corr_p1, .corr_p2, .corr_s1, .corr_s2), 
  .y = paste0("Corr_", rep(c("Pearson_", "Spear_"), each = 2), c("All", "Top")), 
  .f = ~ save_graph(graph = .x, name = .y, width = 10, height = 8)
      )
...
.conf_mat_top_2c <- ggplot(
  mapping = aes(x = reorder(model_code, value), 
                y = value, 
                colour = factor(mode, levels = c("Micro", "Macro", "PHT", "PPGL", "PA", "CS")), 
                group = mode)) +
  stat_summary(data = class_df |> filter(!mode %in% c("PA", "CS" ,"PHT", "PPGL")),
               geom = "line", 
               fun = median,
               linewidth = 2) +
  stat_summary(data = class_df |> filter(mode %in% c("PA", "CS" ,"PHT", "PPGL")),
...
### Testing Top Model Confusion Matrix Differences
...
```{r}
dummy |> 
  summarise(.by = c(model_type, model_id), 
            across(precision:matthew_coef, mean)) |> 
  mutate(
    sample_size = as.numeric(str_extract(model_type, "_(\\d+)_", group = 1)),
    imbalance = as.numeric(str_extract(model_type, "(\\d+)$", group = 1))) |> 
  ggplot(aes(x = imbalance, y = precision, colour = sample_size, group = sample_size)) +
  geom_point(size = 3, shape = 15)+
  geom_line(linewidth = 1.3)+
  ggtitle("Performance of Precision Sensitive to Imbalance in Data")+
  report_theme(12)
...
dummy |> 
  ...
  mutate(
    sample_size = as.numeric(str_extract(model_type, "_(\\d+)_", group = 1)),
    imbalance = as.numeric(str_extract(model_type, "(\\d+)$", group = 1))) |> 
  ggplot(aes(x = sample_size, y = precision, colour = imbalance, group = imbalance)) +
  geom_point(size = 3, shape = 15)+
  geom_line(linewidth = 1.3)+
  ggtitle("Performance of Precision Sensitive to Imbalance in Data")+
  report_theme(12)
...
dummy |> 
  ...
  mutate(
    sample_size = as.numeric(str_extract(model_type, "_(\\d+)_", group = 1)),
    imbalance = as.numeric(str_extract(model_type, "(\\d+)$", group = 1))) |> 
  ggplot(aes(x = imbalance, y = f1, colour = sample_size, group = sample_size)) +
  geom_point(size = 3, shape = 15)+
  geom_line(linewidth = 1.3)+
  ggtitle("Performance of F1-Score Sensitive to Imbalance in Data")+
  report_theme(12)
...
dummy |> 
  ...
  mutate(
    sample_size = as.numeric(str_extract(model_type, "_(\\d+)_", group = 1)),
    imbalance = as.numeric(str_extract(model_type, "(\\d+)$", group = 1))) |> 
  ggplot(aes(x = sample_size, y = f1, colour = imbalance, group = imbalance)) +
  geom_point(size = 3, shape = 15)+
  geom_line(linewidth = 1.3)+
  ggtitle("Performance of F1-Score Sensitive to Imbalance in Data")+
  report_theme(12)
...
dummy |> 
  ...
  mutate(
    sample_size = as.numeric(str_extract(model_type, "_(\\d+)_", group = 1)),
    imbalance = as.numeric(str_extract(model_type, "(\\d+)$", group = 1))) |> 
  ggplot(aes(x = imbalance, y = recall, colour = sample_size, group = sample_size)) +
  geom_point(size = 3, shape = 15)+
  geom_line(linewidth = 1.3)+
  ggtitle("Performance of F1-Score Sensitive to Imbalance in Data")+
  report_theme(12)
