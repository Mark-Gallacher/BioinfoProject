...
```{r}
.conf_mat_top_2c <- ggplot(mapping = aes(x = reorder(model_code, value), y = value, colour = mode, group = mode)) +
...
best_conf_df <- conf_wide |> 
  ...
  right_join(.tmp_best_metrics, by = c("model_id", "model_code")) |> 
...
.conf_mat_top_4 <- best_conf_df |> 
  filter(!model_code %in% c("GNB", "LSCM", "KNN")) |> 
